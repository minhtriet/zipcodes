...
def validate_filename(filename):
    if not os.path.exists(filename):
        raise ValueError(f"Cannot find file {filename} in path {os.getcwd()}")
    if (os.path.exists(src.constants.TEMP_UNMATCH_CSV)) and (filename != src.constants.TEMP_UNMATCH_CSV):
        raise ValueError(f'''Found a file named {src.constants.TEMP_UNMATCH_CSV}, which is a result of the 
            previous\n run. You may want to pass it in instead. To ignore this error, please delete \n
            {src.constants.TEMP_UNMATCH_CSV} first and run the code again.''')
    return filename
